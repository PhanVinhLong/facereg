...
def test_face_detection_recognition():
    ...
    TIME1 = time.time()
    print('Total detect boxes: ', len(boxes))
    print(confidences)
    boxes = boxes[:20]
    ...
    END_TIME = time.time()
    print("dectect: ", TIME1 - START_TIME)
    print("verify: ", END_TIME - TIME1)
...
config_file = "config_files/yolo_config/yolov4-tiny-3l.cfg"
weight_file = "config_files/yolo_config/yolov4-tiny-3l.weights"
model = Darknet(config_file)
model.load_weights(weight_file)
model.cuda()

# on single input image 2
def test_face_detection_recognition_2():
    print("START TEST FACE DETECTION RECOG")
    ...
    for result_sub in result:
        box = [result_sub[0], result_sub[1], result_sub[2]-result_sub[0], result_sub[3] - result_sub[1]]
        boxes.append(box)
        confidences.append(result_sub[5])

    #print("PYTORCH boxes:", boxes)
    TIME1 = time.time()
    print('Total detect boxes: ', len(boxes))
    print(confidences)
    boxes = boxes[:20]
...
test_face_detection_recognition_2()
